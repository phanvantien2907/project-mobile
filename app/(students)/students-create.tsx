...
import {
  createStudent, GENDER_OPTIONS, ACADEMIC_STATUS_OPTIONS, normalizeGpaInput,
  Gender, AcademicStatus,
...
export default function StudentsCreateScreen() {
  ...
  const { control, handleSubmit } = useForm<FormData>({
    defaultValues: {
      ...
      class_name: "",    gpa: "",            academic_status: "studying",
    },
  });

  useEffect(() => {
    navigation.setOptions({ title: "Thêm sinh viên" });
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      await createStudent({
        ...
        gpa:             data.gpa ? parseFloat(normalizeGpaInput(data.gpa)) : undefined,
        academic_status: (data.academic_status as AcademicStatus) || "studying",
        isActive: true,
      });
      Toast.show({ type: "success", text1: "Thành công", text2: "Đã thêm sinh viên" });
      router.back();
    } catch {
