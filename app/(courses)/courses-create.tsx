...
// ─── Screen ──────────────────────────────────────────────────────────────────
export default function CoursesCreateScreen() {
  ...
  useEffect(() => {
    navigation.setOptions({ title: "Thêm Môn học" });
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      Toast.show({ type: "success", text1: "Thành công", text2: "Đã thêm môn học" });
      router.back();
    } catch {
    ...
    >
      ...
      >
        ...
        <View className="mb-6 gap-3">
          <Controller control={control} name="course_code" rules={{ required: true }}
            render={({ field: { onChange, value }, fieldState: { error } }) => (
              <Input placeholder="Mã môn học" value={value} onChangeText={onChange}
          ...
          <Controller control={control} name="course_block"
            render={({ field: { onChange, value } }) => (
              <Select options={COURSE_BLOCK_OPTIONS} value={value || null} onChange={onChange}
                placeholder="Khối kiến thức" icon={GraduationCap} variant="outline" />
            )} />
        ...
        <View className="mb-8 gap-3">
          <Controller control={control} name="semester_period"
            render={({ field: { onChange, value } }) => (
              <Select options={SEMESTER_PERIOD_OPTIONS} value={value || null} onChange={onChange}
                placeholder="Phân kỳ" icon={Layers} variant="outline" />
            )} />
          ...
          <Controller control={control} name="lesson_distribution"
            render={({ field: { onChange, value } }) => (
              <Select options={LESSON_DISTRIBUTION_OPTIONS} value={value || null} onChange={onChange}
                placeholder="Phân tiết" icon={LayoutList} variant="outline" />
            )} />
